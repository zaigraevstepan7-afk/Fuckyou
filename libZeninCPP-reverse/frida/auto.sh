...
# auto.sh — всё-в-одном для Termux (телефон + root): скачивает frida-inject,
...
case "$ARCH" in
  ...
  armeabi*) FA="android-arm";;
  x86_64) FA="android-x86_64";;
  x86) FA="android-x86";;
  *) FA="android-arm64";;
esac
...
echo "[*] пишу combined.js…"
cat > $HOME/combined.js <<'JSEOF'
