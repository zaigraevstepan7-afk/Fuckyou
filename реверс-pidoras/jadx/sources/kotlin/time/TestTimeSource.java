...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1536plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m1446toLongimpl(duration, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m1535overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m1403divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m1446toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m1536plusAssignLRDsOJo(half);
                m1536plusAssignLRDsOJo(Duration.m1435minusLRDsOJo(duration, half));
                return;
    ...
    private final void m1535overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1449toStringimpl(duration)) + '.');
    }
}
